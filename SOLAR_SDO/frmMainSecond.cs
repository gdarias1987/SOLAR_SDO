...
using SOLAR_SDO.Model;
using System.Threading.Tasks;
using System.Threading;

namespace SOLAR_SDO
{
    public partial class frmMainSecond : Form
    {
        ...
        private void NASA_Click(object sender, EventArgs e)
        {
            if (!ctrl)
            {
                ...
                string DateFolder = @"C:\SUN\box\"; // " + DateTime.Now.ToString("yyyy-MM-dd") + @"\" ;

                List<string> fechas = Directory.GetDirectories(DateFolder).ToList();
                foreach (string dia in fechas)
                {
                    List<string> imagenes = Directory.GetFiles(dia + @"\").ToList();
                    data.Add(new DataModel(dia, imagenes));
                }

                //foreach (var imagen in data[0].Imagenes)
                //{
                //    SystemParametersInfo(0x0014, 0, imagen, 0x0001);
                //    Task.Delay(1000);
                //}


                for (int i = 0; i < data[0].Imagenes.Count; i++)
                {
                    SystemParametersInfo(0x0014, 0, data[0].Imagenes[i], 0x0001);
                    Task.Delay(1000);
                }
                for (int i = data[0].Imagenes.Count - 1; i >= 0; i--)
                {
                    SystemParametersInfo(0x0014, 0, data[0].Imagenes[i], 0x0001);
                    Task.Delay(1000);
                }
